...
class TemperaturePredictor:
    def __init__(self, models_folder: str):
        ...
        self.classifier = self._load_classifier(os.path.join(self.models_folder, "classification_model.pth"))
        self.regressors = self._load_regressors(self.models_folder)
    ...
    def _load_regressors(self, models_folder: str):
        """Load regression models for each range"""
        regressors = {}
        range_files = {
            "0 - 10": os.path.join(self.models_folder, "0-10_model.pth"),
            "10 - 30": os.path.join(self.models_folder, "10-30_model.pth"),
            "-10 - 0": os.path.join(self.models_folder, "-10-0_model.pth"),
            "-30 - -10": os.path.join(self.models_folder, "-30--10_model.pth"),
        }
    ...
    async def predict_from_bytes(self, image_bytes: bytes):
        try:
            loop = asyncio.get_event_loop()
            image = await loop.run_in_executor(None, lambda: Image.open(BytesIO(image_bytes)).convert('RGB'))
